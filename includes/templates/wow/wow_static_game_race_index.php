...
<li class="last"><a href="<?php echo WoW::GetWoWPath(); ?>/wow/game/race/" rel="np"><?php echo WoW_Locale::GetString('template_game_race_index'); ?></a></li>
</ol>-->
</div>
<div class="content-bot">
	...
	<?php
      ...
      foreach($races as $faction_name => $races) {
          echo sprintf('<div class="racegroup %s">
		<span class="race-title">%s</span>', $faction_name, WoW_Locale::GetString('template_game_race_'.$faction_name));

          $i = 0;
          foreach($races as $race_name => $race_id) {
              if(in_array($race_name, $exp_races['cataclysm']) ) {
                  $req = sprintf('<em class="class-req cataclysm">%s</em>', WoW_Locale::GetString('template_game_race_req_cataclysm'));
              }
              elseif(in_array($race_name, $exp_races['bc']) ) {
                  $req = sprintf('<em class="class-req bc">%s</em>', WoW_Locale::GetString('template_game_race_req_bc'));
              }
              else {
                  $req = '';
              }
	...
	</div>', $race_name, $race_name, WoW_Locale::GetString('template_game_race_'.$race_name), $req, WoW_Locale::GetString('template_game_race_'.$race_name.'_info') );
              ++$i;
          }
